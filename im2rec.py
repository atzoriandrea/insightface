...
def write_list(path_out, image_list, double_label=None):
    ...
    with open(path_out, 'w') as fout:
        for i, item in enumerate(image_list):
            ...
            for j in item[2:]:
                line += f'{j}\t'
            if double_label:
                line += f'{double_label.index(item[1].split("/")[0])}\t'
            line += f'{item[1]}\n'
            fout.write(line)
...
def make_list(args):
    ...
    if args.double_label:
        demo = set()
        for img in image_list:
            parent = img[1].split("/")[0]
            demo.add(parent)
        demo = sorted(list(demo))
    else:
        demo = None
    N = len(image_list)
    ...
    for i in range(args.chunks):
        ...
        if args.train_ratio == 1.0:
            write_list(args.prefix + str_chunk + '.lst', chunk, demo)
        else:
            if args.test_ratio:
...
def parse_args():
    ...
    cgroup.add_argument('--double_label', action='store_true',
                        help='If true, use the parent in the path to generate a second label ')
